...
- Warnings are treated as errors, **except `DeprecationWarning`**, which is downgraded to `default` (reported, never fatal). Two warnings are silenced outright: astropy `XDG_CONFIG_HOME` (a `UserWarning`, still fatal in every other form) and `datetime.utcfromtimestamp` from the boto3 import chain
