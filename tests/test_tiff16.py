"""Tests for :func:`write_tiff16` and :func:`read_tiff16` round-trip conversion.
...
def test_grayscale_round_trip(tmp_path: Path) -> None:
    ...
    Creates an 8x8 array, writes it with write_tiff16, reads it back with
    read_tiff16, and verifies the data matches.
    """
    arr = (np.arange(64, dtype=np.uint16) * 1000).reshape(8, 8)
    outfile = str(tmp_path / 'gray.tiff')
    write_tiff16(outfile, arr)

    new_arr, palette = read_tiff16(outfile)
...
def test_rgb_round_trip(tmp_path: Path) -> None:
    ...
    write_tiff16, reads it back with read_tiff16, and verifies the data matches.
    """
...
def test_palette_round_trip_translate_true(tmp_path: Path) -> None:
    ...
    write_tiff16(outfile, arr, palette=palette, translate=True)
    new_arr, new_palette = read_tiff16(outfile)
...
def test_palette_round_trip_translate_false(tmp_path: Path) -> None:
    ...
    write_tiff16(outfile, arr, palette=palette, translate=False)
    new_arr, new_palette = read_tiff16(outfile)
...
def test_up_flag_flips_grayscale(tmp_path: Path) -> None:
    ...
    write_tiff16 with up=True flips line ordering on write; read_tiff16 with
    up=True should reverse the flip and produce the original array.
    """
    # `up=True` flips line ordering on write; read_tiff16 with `up=True`
    # should produce the original array.
    arr = (np.arange(16, dtype=np.uint16) * 1000).reshape(4, 4)
    outfile = str(tmp_path / 'gray_up.tiff')
    write_tiff16(outfile, arr, up=True)

    new_arr, _ = read_tiff16(outfile, up=True)
    np.testing.assert_array_equal(new_arr.squeeze(), arr)


def test_invalid_file_raises(tmp_path: Path) -> None:
    """Test that read_tiff16 raises IOError for corrupt TIFF files.

    Creates a file with invalid content and verifies that read_tiff16 raises
...
def test_wrong_tiff_version_raises(tmp_path: Path) -> None:
    ...
    Regression: read_tiff16 built an OSError for this case but did not raise it,
