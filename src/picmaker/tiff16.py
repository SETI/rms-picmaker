...
:func:`write_tiff16` writes a numpy array as a 16-bit uncompressed TIFF in grayscale, RGB,
or palette color. :func:`read_tiff16` reads files written by :func:`write_tiff16`; it is
not a general TIFF reader. Note that 16-bit palette color is not widely supported, so
:func:`write_tiff16` translates it to RGB by default.
"""
...
def write_tiff16(filepath, array, palette=None, up=False, byteorder='native',
    ...
    with open(filepath, 'wb') as f:
        ...
        if byteorder == 'little':
            ...
            raise ValueError('byteorder must be "native", "little", or "big"; '
                             f'got "{byteorder}"')
...
def read_tiff16(filepath, up=False, transpose=None):
