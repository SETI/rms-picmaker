...
class ColorNames:
    ...
    * An RGB container expressed as "(r, g, b)" or "[r, g, b]", where `r`, `g`, and `b`
    ...
    _dict = {k.lower().replace(' ', ''): v for k, v in _COLOR_NAME_DICT.items()}
    _COLOR_NAME_DICT.update(_dict)

    _RGB_PATTERN = re.compile(r'\(?\[?(\d+),(\d+),(\d+)\]?\)?$')
    ...
    def lookup(name):
        ...
        # See if it is an RGB expression in () or []
        match = ColorNames._RGB_PATTERN.fullmatch(test)
        if match:
            rgb = [int(n) for n in match.groups()]
            if any(n > 255 for n in rgb):
                raise ValueError('RGB value cannot exceed 255')
            return tuple(rgb)

        # Look up the color without dashes, underscores, quotes
        test = test.replace('-', '').replace('_', '') .replace('"', '').replace("'", '')
